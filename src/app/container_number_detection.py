...
class ContainerNumberPrediction:
	...
	def filter_and_crop(self, img, results, min_confidence=0.0):
		...
		return image_crop_max, confidence_list[confidence_max_index], bbox_list[confidence_max_index]

	def draw_boxes(self, image, bbox, width=1):
		'''
		Draw ractangle bbox prediction
		and return PIL Image
		'''
		color = (45, 255, 255)
		image_draw = cv2.rectangle(image, (bbox[0]-10, bbox[1]-10), (bbox[2]+10, bbox[3]+10), color, width)
		image_draw_pill = Image.fromarray(cv2.cvtColor(image_draw,cv2.COLOR_BGR2RGB))
		return image_draw_pill
